...
* Other instructor: Bryan O'Sullivan
    * Implemented many key Haskell libraries, co-wrote
...
* Course assistant: David Terei
    ...
    * Implemented [Safe Haskell][SafeHaskell] and GHC LLVM backend
...
-->
    * Some are applicable to other languages (though returning to
      other languages after Haskell can be frustrating)
...
* First week cover Haskell basics
    * If you haven't used Haskell, you should supplement by reading
      ...
      <http://www.haskell.org/tutorial/> or
...
* Final grade will be based on several factors
    1. Class attendance and participation
        * non-SCPD students should attend lecture in person
          **<span style="color:red">and bring laptops to class</span>**
        * SCPD students should participate in discussions on Piazza
    1. Three small warm-up solo programming exercises
        * **<span style="color:red">HW1 due one week from today</span>**
    1. A large final group project & presentation
...
* Install [Stack][Stack] and optionally the
  [Haskell platform][Platform] (includes [GHC][GHCdoc])
...
* Unlike variables in imperative languages, Haskell bindings are
    * *immutable* - can only bind a symbol once in a give scope (bound
      symbols still called "variables" since function arguments can
      vary across invocations)
...
* Inner functions (`loop`) often have arguments related to
  outer function
...
* Personally, I find this practice a bit error-prone
    ...
    * So can avoid problem by using longer symbol name for the outer
      function (i.e., shorter name for shorter scope)
...
* Some basic types:
    ...
    * `(`*type1*`,` *type2*`,` ...`,` *typeN*`)` - a tuple (where $N\ne 1$)
...
* The `data` keyword declares user-defined data types (like `struct`
  in C):
    ...
    * Declares new type, `PointT` with constructor
...
* Directly match keywords:
    ...
    * But this solution too finicky---won't accept trailing carriage
...
* If `length` tail recursive, `countLowerCase` might run in constant space
    ...
    * But until each of the *head* or *tail* is needed, it can be
      stored as a function pointer

    * So `length` will cause `filter` to produce `Char`s one at a time
...
* The "`.`" operator provides function composition
    ...
    * "`f . g`" is an ASCII approximation of mathematical "$f\circ g$"
...
* Haskell allows anonymous functions through *lambda abstraction*
    ...
    * "`\`" is an ASCII approximation of "$\lambda$", so pronounced
      "lambda"
...
* Most operators are just library functions in Haskell
    * Very few operators reserved by language syntax<br>
      (`..`, `:`, `::`, `=`, `\`, `|`, `<-`, `->`, `@`, `~`, `=>`, `--`)
...
* <span class="emph">`$`</span> is function application, but
...
* <span class="emph">`seq :: a -> b -> b`</span> just returns
  value of second argument...<br> but forces evaluation of the first
  argument before evaluating the second
...
* <span class="emph">`$!`</span> combines `$` and `seq`
...
* Unfortunately, `acc` can contain a chain of thunks `n` long<br>
    ...
    * GHC is smart enough not to build up thunks, but only if
      optimizing
...
* [Haskell stack][Stack] is a tool to make it easier to manage
  packages
* I highly recommend reading
  [the User guide](http://docs.haskellstack.org/en/stable/GUIDE.html)
* Some basic commands
    * `stack new` _mypackage_ [`new-template`] -- generate scaffolding
    * `stack setup` -- install GHC, etc. (required even if you have a
      system GHC)
    * `stack build` -- build your project
    * `stack exec -- $SHELL` -- spawn a shell in which you can run
      your program
    * `stack haddock` _somepackage_ -- install documentation for
      _somepackage_
    * `$BROWSER $(stack path --snapshot-doc-root)/index.html` -- view
      system documentation
    * `$BROWSER $(stack path --local-doc-root)/index.html` -- view
      your documentation
    * `stack unpack` _somepackage_ -- download source for
      _somepackage_
* You may want to edit
  [`$HOME/.stack/config.yaml`](https://github.com/commercialhaskell/stack/blob/release/doc/yaml_configuration.md)
...
* Type the following into a file `greet.hs`:
    ...
    * Or shortcut, type: `wget`
      [`cs240h.stanford.edu/greet1.hs`](http://cs240h.stanford.edu/greet1.hs)
...
* A `do` block lets you sequence IO actions.  In a `do` block:
    * <span class="emph">*pat* `<-` *action*</span> - binds
      *pat* (variable or constructor pattern) to result of executing
      *action*
    * <span class="emph">`let` *pat* `=` *pure-value*</span> -
    binds *pat* to *pure-value* (no "`in` ..." required)
    * <span class="emph">*action*</span> - executes *action* and
...
Prelude Main>
    ...
    * No `*` before `Main` means compiled.  For access to internal
      symbols, say:
...
* What if we want `greet` to return the name of the person?
    ...
    * Solution: `return` function gives trivial `IO` action returning
      a particular value
...
* Starter code:  `wget`
  [`cs240h.stanford.edu/rock1.hs`](http://cs240h.stanford.edu/rock1.hs)
...
* Answer: in this case, compiler is not actually that smart
    * Heuristic: If it looks like a function, can infer *ad hoc*
      polymorphic types
    * If it looks like a constant, no *ad hoc* polymorphism unless
      explicitly declared
...
* Lists are a `Functor`
    ...
    * `[]` can be used as a prefix type constructor ("`[] Int`" means
      "`[Int]`") and can be used to declare instances
...
* Haskell let define [field labels][FieldLabels] (like C `struct`s)
...
* There is a special syntax for updating one or more fields
    ...
    * Another idiom:  construct values by customizing some
      `defaultValue`, makes it easy to add more fields later on
...
* A `!` before a data field type makes it *strict* - i.e., can't be
  thunk
    ...
    * When initializing a datatype using labels, it is mandatory to
...
* Instead of `withTty`, let's define `withClient` that uses TCP:
    ...
    * To get code:  `wget`
      [`cs240h.stanford.edu/rock2.hs`](http://cs240h.stanford.edu/rock2.hs)
...
[FieldLabels]: https://www.haskell.org/onlinereport/haskell2010/haskellch3.html#x8-490003.15
